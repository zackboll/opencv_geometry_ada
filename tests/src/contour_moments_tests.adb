...
package body Contour_Moments_Tests is
   ...
   procedure Assert_Zero
     (Value : OpenCV.Geometry.Moments_Result; Message : String) is
   begin
   ...
   procedure Translation_Invariance (Test : in out Fixture) is
      ...
      Original : constant OpenCV.Geometry.Moments_Result :=
        OpenCV.Geometry.Compute_Moments (Rectangle);
      Shifted  : constant OpenCV.Geometry.Moments_Result :=
        OpenCV.Geometry.Compute_Moments (Translated);
   begin
      Assert_Close
   ...
   procedure Degenerate_Contours (Test : in out Fixture) is
      ...
      One_Moments   : constant OpenCV.Geometry.Moments_Result :=
        OpenCV.Geometry.Compute_Moments (One_Point);
      Two_Moments   : constant OpenCV.Geometry.Moments_Result :=
        OpenCV.Geometry.Compute_Moments (Two_Points);
   begin
   ...
   procedure Nonzero_Array_Bounds (Test : in out Fixture) is
      pragma Unreferenced (Test);
      Offset_Rectangle : constant OpenCV.Geometry.Contour (7 .. 10) :=
        (7  => (X => 0, Y => 0),
         8  => (X => 4, Y => 0),
         9  => (X => 4, Y => 3),
         10 => (X => 0, Y => 3));
      Ordinary         : constant OpenCV.Geometry.Moments_Result :=
        OpenCV.Geometry.Compute_Moments (Rectangle);
      Offset           : constant OpenCV.Geometry.Moments_Result :=
        OpenCV.Geometry.Compute_Moments (Offset_Rectangle);
   begin
   ...
   procedure C_ABI_Validation (Test : in out Fixture) is
      pragma Unreferenced (Test);
      Value  : aliased C_API.C_Moments := (others => -1.0);
      Status : C_API.Status;
   begin
      ...
      Assert_C_Zero (Value, "invalid count must clear every moment");
      Value := (others => -1.0);
      Status := C_API.Contour_Moments (null, 1, Value'Access);
      AUnit.Assertions.Assert
        (Status = C_API.Error_Invalid_Argument
      ...
      Assert_C_Zero (Value, "invalid points must clear every moment");
